...
# db connection
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqldb://flask:f1ask@db_server/url_db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)

# Generates the short
def url_generator(size=5, chars=string.ascii_uppercase + string.digits):
...
def add_url():
    ...
    if request.method == 'POST':
        ...
        if urlparse(originalUrl).scheme == '': # if no http://
